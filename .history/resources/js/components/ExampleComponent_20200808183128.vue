...
<script>
    export default {
        ...
        methods:{
            getTasks(){
                let me =this;
                let url = this.url_base+'/tareas' //Ruta que hemos creado para que nos devuelva todas las tareas
            ...
            saveTasks(){
                let me =this;
                let url = this.url_base+'/tareas/guardar' //Ruta que hemos creado para enviar una tarea y guardarla
            ...
            updateTasks(){/*Esta funcion, es igual que la anterior, solo que tambien envia la variable update que contiene el id de la
                ...
                let me = this;
                axios.put(this.url_base+'/tareas/actualizar',{
                    'id':this.update,
                    'name':this.name,
                    'description':this.description,
                    'content':this.content,
            ...
            loadFieldsUpdate(data){ //Esta función rellena los campos y la variable update, con la tarea que queremos modificar
                this.update = data.id
                let me =this;
                let url = this.url_base+'/tareas/buscar?id='+this.update;
                axios.get(url).then(function (response) {
            ...
            deleteTask(data){//Esta nos abrirá un alert de javascript y si aceptamos borrará la tarea que hemos elegido
                ...
                if (confirm('¿Seguro que deseas borrar esta tarea?')) {
                    axios.delete(this.url_base+'/tareas/borrar/'+task_id
                    ).then(function (response) {
                        me.getTasks();
                    })
                    .catch(function (error) {
